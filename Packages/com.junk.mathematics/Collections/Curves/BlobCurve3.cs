...
namespace Junk.Collections
{
    ...
    public struct BlobCurve3 : IBlobCurve<float3>
    {
        ...
        public static ref BlobCurve3 Create(ref BlobBuilder builder, ref BlobCurve3 data, AnimationCurve curveX, AnimationCurve curveY, AnimationCurve curveZ,
                                        Allocator       allocator = Allocator.Persistent)
        {
            ...
            if (hasOnlyOneKeyframe)
            {
                ...
                var key0y = yKeys[0];
                var key0z = zKeys[0];
                builder.Allocate(ref data.Segments, 1)[0] = new BlobCurveSegment3(key0x, key0y, key0z, key0x, key0y, key0z);
            ...
            else
            {
                ...
                var segmentBuilder  = builder.Allocate(ref data.Segments, segmentCount);
                for (int i = 0, j = 1; j < keyFrameCount; i = j++)
                {
                    timeBuilder[j] = xKeys[i].time;
                    segmentBuilder[i] = new BlobCurveSegment3(
