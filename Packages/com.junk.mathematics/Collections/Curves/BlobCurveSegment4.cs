...
Junk.Collections
{
    ...
    public struct BlobCurveSegment4
    {
        ...
        public float4 Sample(float4 timeSerial) => math.mul(timeSerial, this.Factors);
        ...
        {
            Assert.IsTrue(
                Mathf.Approximately(k0x.time, k0y.time) && Mathf.Approximately(k1x.time, k1y.time) &&
                Mathf.Approximately(k0x.time, k0z.time) && Mathf.Approximately(k1x.time, k1z.time) &&
                Mathf.Approximately(k0x.time, k0w.time) && Mathf.Approximately(k1x.time, k1w.time),
                "Time not sync");
        ...
        public static BlobCurveSegment4 Unity4(float4 value0, float4 tangent0, float4 tangent1, float4 value1, float duration)
            => new(
                BlobShared.UnityFactor(value0.x, tangent0.x, tangent0.x, value1.x, duration),
        ...
        public static BlobCurveSegment4 Bezier4(float4 value0, float4 cv0, float4 cv1, float4 value1)
            => new(
                BlobShared.BezierFactor(value0.x, cv0.x, cv1.x, value1.x),
        ...
        public static BlobCurveSegment4 Linear4(float4 value0, float4 value1)
            => new(
                BlobShared.LinearFactor(value0.x, value1.x),
                BlobShared.LinearFactor(value0.y, value1.y),
                BlobShared.LinearFactor(value0.z, value1.z),
                BlobShared.LinearFactor(value0.w, value1.w));
    }
}
